...
export function update() {
    let nodeValues = Global.nodes.entries(),
        nodes = Global.svg.mmp.selectAll(".node").data(nodeValues),
        branches = Global.svg.mmp.selectAll(".branch").data(nodeValues.slice(1)),
    ...
    if (Global.options["drag"] === true) outer.call(node.drag);
    ...
    outer.insert("foreignObject")
        .html(n => `<div style="
            ...
            font-family: ${Global.options["font-family"]};
            text-align: center;
...
/**
 * @name redraw
 * @desc Remove getDom elements and recreate them.
 */
